...
<template>
  <div>
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <tr :key="index" v-for="(stockList, index) in stock_inventories ">
          <!--stock_inventories-->
          <td>{{index + 1}}</td>
          <td width="10%">
            <input
          ...
          <td width="8%">
            ...
            <span
              ...
              v-show="`sale_price${index}`"
            >{{ errors.first(`sale_price${index}`) }}</span>
            <!--            v-validate="'required'"-->
          </td>
          <td width="8%">
            <input
            ...
            <span
              class="help-block text-danger"
              v-show="`mrp_price${index}`"
            >{{ errors.first(`mrp_price${index}`) }}</span>
          </td>
          <td width="8%">
            <input
            ...
            <span
              class="help-block text-danger"
              v-show="`stock_available${index}`"
            >{{ errors.first(`stock_available${index}`) }}</span>
          </td>
          ...
          <td width="5%">
            <input
              ...
              type="text"
              v-model="stockList.length"
              v-validate="{required:true}"
            />
            <span
              aria-hidden="true"
              class="glyphicon glyphicon-remove form-control-feedback"
              v-show="errors.has(`length${index}`)"
            ></span>
            <span
              class="help-block text-danger"
              v-show="`length${index}`"
            >{{ errors.first(`length${index}`) }}</span>
          </td>
          ...
          <td width="5%">
            <input
              ...
              type="text"
              v-model="stockList.breadth"
              v-validate="{required:true}"
            />
            <span
              aria-hidden="true"
              class="glyphicon glyphicon-remove form-control-feedback"
              v-show="errors.has(`breadth${index}`)"
            ></span>
            <span
              class="help-block text-danger"
              v-show="`breadth${index}`"
            >{{ errors.first(`breadth${index}`) }}</span>
          </td>
          <td width="5%">
            <input
              :name="'height' +index "
              ...
              type="text"
              v-model="stockList.height"
              v-validate="{required:true}"
            />
            <span
              aria-hidden="true"
              class="glyphicon glyphicon-remove form-control-feedback"
              v-show="errors.has(`height${index}`)"
            ></span>
            <span
              class="help-block text-danger"
              v-show="`height${index}`"
            >{{ errors.first(`height${index}`) }}</span>
          </td>
          <td width="5%">
            <input
              :name="'weight' +index "
              ...
              type="text"
              v-model="stockList.weight"
              v-validate="{required:true}"
            />
            <span
              aria-hidden="true"
              class="glyphicon glyphicon-remove form-control-feedback"
              v-show="errors.has(`weight${index}`)"
            ></span>
            <span
              class="help-block text-danger"
              v-show="`weight${index}`"
            >{{ errors.first(`weight${index}`) }}</span>
          </td>
  ...
  methods: {
    ...
    async handleFileChange(index) {
      ...
      if (res && res.success && res.success === true) {
        if (
          this.stock_inventories[index].images &&
          this.stock_inventories[index].images.length
        ) {
